...
beans = {
     exceptionHandler(DialogExceptionResolver) {
...
grails.databinding.dateFormats = ["yyyy-MM-dd'T'HH:mm:ss'Z'"]
    ...
    // URLmappings for error pages
    def urlMappingsPath="${basedir}/grails-app/conf/UrlMappings.groovy"
    def urlMappingsText= new File(urlMappingsPath).text
    urlMappingsText=StringUtils.replace(urlMappingsText,'''"500"(view:'/error')''','''
"500" (controller: "dialogException", action: "dialog", exception: org.open_t.dialog.DialogException)
"500" (controller: "dialogException", action: "dialog", exception: Exception)''')

    new File(urlMappingsPath).write(urlMappingsText)
